...
def layer_conv_bn_relu(layer_input, filters, nb_row, nb_col, nb_regularizer):
    ...
    layer_process = BatchNormalization(axis=-1, epsilon=1e-7,
...
class UISRCNNConfig(BaseConfig):
    def __init__(self):
        super(UISRCNNConfig, self).__init__()
...
class UISRCNN_Model(SR_Model):
    def __init__(self, config):
        super(UISRCNN_Model, self).__init__(config)
    ...
    def add_model(self, input_data, target_data=None):
        ...
        # module-1
        # conv/BN/ReLU-1
        x = layer_conv_bn_relu(input_data, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-2
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-3
        x_1 = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)

        # module-2
        # conv/BN/ReLU-1
        x = layer_conv_bn_relu(x_1, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-2
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-3
        x_2 = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)

        # module-3
        # conv/BN/ReLU-1
        x = layer_conv_bn_relu(x_2, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-2
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-3
        x_3 = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)

        # module-4
        # conv/BN/ReLU-1
        x = layer_conv_bn_relu(x_3, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-2
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-3
        x_4 = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)

        # module-5
        # conv/BN/ReLU-1
        x = layer_conv_bn_relu(x_4, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-2
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        # conv/BN/ReLU-3
        x = layer_conv_bn_relu(x, self.config.n1, self.config.f1, self.config.f1, self.config.regularizer)
        ...
        x = BatchNormalization(axis=-1, epsilon=1e-7,
                               gamma_initializer='one',
                               beta_initializer='zero',
                               weights=None
                               )(x)
